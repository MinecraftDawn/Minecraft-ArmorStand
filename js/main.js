...
var mmLeftLegSkin = new THREE.Mesh();
var mmLeftBootSkin= new THREE.Mesh();;
var mmRightLegSkin= new THREE.Mesh();;
var mmRightBootSkin= new THREE.Mesh();;
var mmBodySkin= new THREE.Mesh();;
var mmWaistSkin= new THREE.Mesh();;
var mmLeftArmSkin= new THREE.Mesh();;
var mmRightArmSkin= new THREE.Mesh();;

var materials = {};

var typeMap = {"chainmail":"chainmail",
				"diamond":"diamond",
				"golden": "gold",
				"netherite":"netherite",
				"iron": "iron",
...
function initMaterials(){
	let materialTypes = ['chainmail','gold','iron','leather','netherite','diamond'];
	let armorTypes = ['helmets','chestplates','leftBoots','rightBoots','leftArms','rightArms','leftLegs','rightLegs','waists'];
	materialTypes.forEach(x => {
		materials[x] = {};
		armorTypes.forEach(xx => {
			armorName = xx.substr(0, xx.length-1);
			skins = [];
			for(let i=0; i<6; i++){
				let texture = new THREE.TextureLoader().load(`./images/${x}/${armorName}${i}.png`);
				texture.magFilter = THREE.NearestFilter;
				let material = new THREE.MeshBasicMaterial({map:texture, side:THREE.DoubleSide, transparent:true});
				skins.push(material);
			}
			materials[x][xx] = skins;
		});
	});	
}
...
function setup(){
	...
	mLegLeft.add(mmLeftLegSkin);
	mLegLeft.add(mmLeftBootSkin);
	...
	mLegRight.add(mmRightLegSkin);
	mLegRight.add(mmRightBootSkin);
	...
	mBody.add(mmBodySkin);
	mBody.add(mmWaistSkin);
...
function getMaterial(itemName){
	index = itemName.indexOf("_");
	
	return index == -1 ? "" : typeMap[itemName.substr(0, index)];
}
...
function updateUI(){
	...
	//Set Visibility
	mmArmRight.visible = mmArmLeft.visible = showArms;
	mBasePlate.visible = !noBasePlate;
	mSkull.visible = equipHelmet != "";
	
	mmLeftLegSkin.visible = equipLeggings != "";
	mmLeftBootSkin.visible = equipShoes != "";
	mmRightLegSkin.visible = equipLeggings != "";
	mmRightBootSkin.visible = equipShoes != "";
	mmWaistSkin.visible = equipChestplate != "";
	mmBodySkin.visible = equipChestplate != "";
	mmLeftArmSkin.visible = equipChestplate != "";
